...
import { updateRegistration, deleteRegistration, getTeamMembers, getRegistrationDetails } from "@/app/actions/admin-registrations";
import { X, Check, Save, Trash2, Edit, Eye, Loader2 } from "lucide-react";
...
export default function RegistrationModal({ isOpen, onClose, registrationId, onUpdate, onDelete }: any) {
            ...
            getRegistrationDetails(registrationId).then(({ data }) => {
                if (data) {
                    ...
                    setEditData({
                        ...
                        leader_mobile: data.leader_mobile,
                    ...
                    if (data.registration_type === "team") {
                        ...
                        getTeamMembers(registrationId).then((tmRes) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-md">
            <div className="bg-card/95 w-full max-w-5xl max-h-[90vh] flex flex-col rounded-2xl border border-border/50 shadow-2xl relative overflow-hidden animate-in zoom-in-95 duration-300">
                ...
                {/* Scrollable Body */}
                <div className="overflow-y-auto flex-1 p-6 relative min-h-[400px]">
                    ...
                    {isLoadingData ? (
                        <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-background/50 backdrop-blur-sm">
                            ...
                            <p className="text-muted-foreground font-medium">Loading Registration Details...</p>
                        </div>
                    ) : registration ? (
                                    ...
                                    <div className="flex-1 flex flex-col min-h-[300px]">
                                        ...
                                        {registration.payment_screenshot_url ? (
                                            <div
                                                ...
                                                onClick={() => window.open(`/api/proxy-image?url=${encodeURIComponent(registration.payment_screenshot_url)}`, "_blank")}
                                            >
                                                ...
                                                <img
                                                    src={`/api/proxy-image?url=${encodeURIComponent(registration.payment_screenshot_url)}`}
